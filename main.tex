...
equivalence between $\pi_1|\Ch(Z)|$ and braids, where $Z$ is the terminal precubical set.
...
the construction of the salvetti complex does generalize well.

These minimal faces are called chambers or topes, in hyperplane arrangements.
...
So indeed this circumbents the issue with euclidean complexes from {\cite[Prop.~6.1]{PZ}}.
...
Defining $\Chs$ abstractly has some conveniences. For example,
...
\begin{proof}
  ...
  to $(y_j)_*^{-1} \circ (f_i)_* \circ (x_i)_*$, which by \eqref{eq:rho-restrict} is the increasing
